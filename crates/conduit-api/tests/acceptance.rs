...
use conduit_api::config::ConduitApiConfig;
...
/// Use `ConduitApiConfig::default()` value if no config or args
#[test]
fn start_no_args() {
...
fn start_with_config_no_args() {
    let mut config = ConduitApiConfig::default();
...
#[test]
fn can_print_my_name() {
    let mut runner = RUNNER.clone();
    let mut cmd = runner
        .args(["start", "nguyen", "thanh", "dat"])
        .capture_stdout()
        .run();
    cmd.stdout().expect_line("Hello, nguyen thanh dat!");
    cmd.wait().unwrap().expect_success();
}
